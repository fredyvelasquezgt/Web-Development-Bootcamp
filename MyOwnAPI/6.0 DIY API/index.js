...
app.get("/jokes/:id", (req, res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id === id); //takes array, loop through the item and checks against a particular condition
  res.json(foundJoke);
})
...
app.get("/filter", (req, res) => {
  ...
  const filteredActivities = jokes.filter((joke) => joke.jokeType === type); //filter returns a new array with the things that are true
...
//6. PATCH a joke - updates just a small parat
...
app.delete("jokes/:id", (req, res) => {
  ...
  if(searchIndex > -1) {
    jokes.splice(searchIndex, 1); //removes one element from the jokes array based on the searchIndex 
    res.sendStatus(200)
  } else {
    res
      .status(404)
      .json({error: `Joke with  the id: ${id} not found. No jokes detected`})
  }
})
